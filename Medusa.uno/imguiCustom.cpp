...
/*void ImGui::Checkbox(const char* str_id, bool* v)
{
    ImColor colorbar = Helpers::calculateColor(config->menu.accentColor);
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    const ImGuiStyle& style = ImGui::GetStyle();
    static float r = .06f, g = .06f, b = .06f;
    r = config->menu.accentColor.color[0];
    g = config->menu.accentColor.color[1];
    b = config->menu.accentColor.color[2];
    auto text = str_id;
    ImDrawList* draw_list = ImGui::GetWindowDrawList();
    float height = ImGui::GetFrameHeight();
    float width = height * 1.55f;
    const ImVec2 p = ImGui::GetCursorScreenPos();
    const ImRect fuckme(p, ImVec2(p.x + height, p.y + height));
    float radius = height * 0.50f;
    if (ImGui::InvisibleButton(str_id, ImVec2(width, height)))
        *v = !*v;
    draw_list->AddRectFilled(p, ImVec2(p.x + width, p.y + height), IM_COL32(22, 22, 22, 255), height * 0.2f);
    ImVec2 center = ImVec2(radius + (*v ? 1 : 0) * (width - radius * 2.0f), radius);
    if (*v)
    {
        draw_list->AddRectFilled(ImVec2((p.x + center.x) - 9.0f, p.y + 1.5f),
            ImVec2((p.x + (width / 2) + center.x) - 9.0f, p.y + height - 1.5f), colorbar, height * 0.2f);
    }
    else
        draw_list->AddRectFilled(ImVec2((p.x + center.x) - 9.0f, p.y + 1.5f),
            ImVec2((p.x + (width / 2) + center.x) - 9.0f, p.y + height - 1.5f), IM_COL32(60, 60, 60, 255), height * 0.2f);
    ImGui::PushID(1);
    ImGui::SameLine();
...
void ImGuiCustom::colorPicker(const char* name, float color[3], float* alpha, bool* rainbow, float* rainbowSpeed, bool* enable, float* thickness, float* rounding, bool* outline) noexcept
{
    ...
    bool openPopup = ImGui::ColorButton("##btn", ImVec4{ color[0], color[1], color[2], alpha ? *alpha : 1.0f }, ImGuiColorEditFlags_NoTooltip | ImGuiColorEditFlags_AlphaPreviewHalf);
    //g.Style.FrameRounding = 7.5f;
    if (enable)
    {
    ...
    if (openPopup)
        ImGui::OpenPopup(c_xor("##popup"));

    if (ImGui::BeginPopup(c_xor("##popup"))) {
        if (alpha) {
            float col[]{ color[0], color[1], color[2], *alpha };
            //| ImGuiColorEditFlags_DisplayHex
            //ImGuiColorEditFlags_AlphaPreviewHalf | ImGuiColorEditFlags_AlphaBar
            ImGui::ColorPicker4(c_xor("##picker"), col, ImGuiColorEditFlags_NoSidePreview | ImGuiColorEditFlags_AlphaPreviewHalf | ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_DisplayHex);
            color[0] = col[0];
            color[1] = col[1];
            color[2] = col[2];
            *alpha = col[3];
            ...
            ImGui::ColorPicker3(c_xor("##picker"), color, ImGuiColorEditFlags_NoSidePreview | ImGuiColorEditFlags_DisplayHex);
        }
...
void ImGuiCustom::colorPicker(const char* name, Color3& colorConfig) noexcept
{
    colorPicker(name, colorConfig.color.data(), nullptr, &colorConfig.rainbow, &colorConfig.rainbowSpeed);
}
...
void ImGui::hotkey2(const char* label, KeyBind& key, float samelineOffset, const ImVec2& size, bool border) noexcept
{
    ...
    else
    {
        if (key.keyMode == KeyMode::Always)
        {
            ...
            ImGui::ButtonEx("On", {}, ImGuiButtonFlags_Disabled, true);
            ImGui::PopStyleColor();
        }
        else if (key.keyMode == KeyMode::Off)
        {
            ImGui::PushStyleColor(ImGuiCol_Text, ImGui::GetColorU32(ImGuiCol_TextDisabled));
            ImGui::ButtonEx("Off", {}, ImGuiButtonFlags_Disabled, true);
            ImGui::PopStyleColor();
        }
        else if (key.isSet())
        {
