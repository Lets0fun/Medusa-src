...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, bool border)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (border == true)
    {
        ImColor cevaPl{ 14, 14, 14, 255 };
        col = GetColorU32((held && hovered) ? Helpers::calculateColor(config->menu.accentColor, 0.5f) : hovered ? Helpers::calculateColor(config->menu.accentColor, 0.25f) : Helpers::calculateColor(14,14,14,255));
    }
    else
    ...
    if (border == true)
    window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(Helpers::calculateColor(config->menu.accentColor, (hovered ? 1.f / 2.f + 0.75f : 1.f / 2.f))), style.FrameRounding);
    if (g.LogEnabled)
...
{
    return ButtonEx(label, size_arg, ImGuiButtonFlags_None, true);
}

bool ImGui::Button1(const char* label, const ImVec2& size_arg)
{
    return ButtonEx(label, size_arg, ImGuiButtonFlags_None, false);
}
...
bool ImGui::Checkbox(const char* name, bool* callback)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderFrame(check_bb.Min, check_bb.Max, Helpers::calculateColor(14, 14, 14, 255), true, 3.f);
    ImU32 check_col = Helpers::calculateColor(0,0,0,255);
    window->DrawList->AddRect(check_bb.Min, check_bb.Max, Helpers::calculateColor(config->menu.accentColor, hovered ? 1.f : 0.5f), 3.f, 0, 1.f);

    if (*callback)
    {
        RenderFrame(check_bb.Min, check_bb.Max, Helpers::calculateColor(config->menu.accentColor), true, 3.f);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, bool text)
{
    ...
    const float size = CalcItemWidth();
    float textY = CalcTextSize(label).y + 4;
    const ImRect rect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(size, text ? height + textY : height));
    const ImRect clickable(window->DC.CursorPos + ImVec2(0, text ? textY : 0), window->DC.CursorPos + ImVec2(size, text ? height + textY : height));
    ...
    window->DrawList->AddRect(clickable.Min, clickable.Max, Helpers::calculateColor(config->menu.accentColor, hovered ? 1.f / 2.f + 0.75f : 1.f / 2.f), 3.f);
    ...
    window->DrawList->AddCircleFilled(ImVec2(rect.Max.x, rect.Max.y) - ImVec2(11, height/2), 3.f + (popup_open ? 1.5f : 0.f), Helpers::calculateColor(config->menu.accentColor));
    ...
    PushStyleColor(ImGuiCol_Border, ImVec4(ImColor(Helpers::calculateColor(config->menu.accentColor, hovered ? 1.f / 2.f + 0.75f : 1.f / 2.f))));
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items, bool text)
{
    ImGuiContext& g = *GImGui;
    ...
    if (!BeginCombo(label, preview_value, ImGuiComboFlags_None, text))
        return false;
...
bool ImGui::Combo(const char* label, int* current_item, const char* const items[], int items_count, int height_in_items, bool text)
{
    const bool value_changed = Combo(label, current_item, Items_ArrayGetter, (void*)items, items_count, height_in_items, text);
    return value_changed;
}

// Combo box helper allowing to pass all items in a single string literal holding multiple zero-terminated items "item1\0item2\0"
bool ImGui::Combo(const char* label, int* current_item, const char* items_separated_by_zeros, int height_in_items, bool text)
{
    int items_count = 0;
    ...
    bool value_changed = Combo(label, current_item, Items_SingleStringGetter, (void*)items_separated_by_zeros, items_count, height_in_items, text);
    return value_changed;
}
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    RenderNavHighlight(frame_bb, id);
    //window->DrawList->AddRectFilled(frame_bb.Min, { frame_bb.Max.x , frame_bb.Max.y + 10.6f }, GetColorU32(ImGuiCol_ChildBg), 5.0f);//RenderFrame(ImVec2{ frame_bb.Min.x , frame_bb.Min.y }, ImVec2{ frame_bb.Max.x , frame_bb.Max.y }, GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), true, 5.f);
    window->DrawList->AddRectFilled({ frame_bb.Min.x, frame_bb.Min.y }, { frame_bb.Max.x, frame_bb.Max.y }, GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), 5.0f);
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(grab_bb.Min.x + 1.5f, frame_bb.Max.y), GetColorU32(ImGuiCol_SliderGrabActive), g.Style.FrameRounding);
    window->DrawList->AddRectFilled(ImVec2(grab_bb.Min.x, grab_bb.Min.y - g.Style.FramePadding.y), ImVec2(grab_bb.Max.x, grab_bb.Max.y + g.Style.FramePadding.y), ImColor(1.0f, 1.0f, 1.0f), g.Style.FrameRounding);
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    if (p_step != NULL)
    {
        ...
        if (ButtonEx("-", ImVec2(button_size, button_size), button_flags, true))
        {
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (is_multiline)
    {
        ...
        PushStyleColor(ImGuiCol_ChildBg, IM_COL32(11,11,11,255));
    ...
    {
        RenderNavHighlight(frame_bb, id); 
        RenderFrame(frame_bb.Min, frame_bb.Max, IM_COL32(11,11,11,255), true, style.FrameRounding);
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(!hovered ? Helpers::calculateColor(config->menu.accentColor, 0.5f) : Helpers::calculateColor(config->menu.accentColor)) ,style.FrameRounding);
    }
    ...
    if (render_cursor || render_selection)
    {
        ...
        gui->isTypingBool = render_selection;
        if (render_selection)
        {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    draw_window->DrawList->AddRect(rect.Min, rect.Max, GetColorU32(ImVec4(ImColor(Helpers::calculateColor(config->menu.accentColor, (hovered ? 1.f / 2.f + 0.75f : 1.f / 2.f))))));
                }
