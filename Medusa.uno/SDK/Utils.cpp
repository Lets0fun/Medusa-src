...
int getMaxUserCmdProcessTicks() noexcept
{
    ...
    if (const auto gameRules = (*memory->gameRules); gameRules)
        ticks = (gameRules->isValveDS()) ? 8 : 16;
    return ticks;
}

GameMode getGameMode() noexcept
{
    static auto gameType{ interfaces->cvar->findVar(skCrypt("game_type")) };
    static auto gameMode{ interfaces->cvar->findVar(skCrypt("game_mode")) };
