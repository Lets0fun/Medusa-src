...
void GrenadePrediction::run(UserCmd* cmd) noexcept
{
	...
	if (lastPos.notNull()) {
		int radius = activeWeapon->itemDefinitionIndex2() == WeaponId::Decoy || activeWeapon->itemDefinitionIndex2() == WeaponId::Flashbang ? 45 : 145;
		drawCircle(lastPos, 120, radius);
		//drawParticles(lastPos);
...
void GrenadePrediction::draw() noexcept
{
	...
	const auto glowColor = Helpers::calculateColor(config->misc.nadeGlowPredict);

	auto drawList = ImGui::GetBackgroundDrawList();
	//	draw glow path
	if (config->misc.nadeGlowPredict.enabled)
	{
		for (auto& point : savedPoints)
			drawList->AddLine(ImVec2(point.first.x, point.first.y), ImVec2(point.second.x, point.second.y), glowColor, 4.0f);

		for (auto& point : endPoints)
			drawList->AddLine(ImVec2(point.first.x, point.first.y), ImVec2(point.second.x, point.second.y), glowColor, 4.f);
	}
	...
	//	draw nade path
	for (auto& point : savedPoints)
		drawList->AddLine(ImVec2(point.first.x, point.first.y), ImVec2(point.second.x, point.second.y), trailColor, 2.0f);
	...
	if (config->misc.nadeDamagePredict.enabled)
	{
		for (auto& point : dmgPoints)
		{
			...
			drawList->AddText({ point.first.x - horizontalOffset + 1.0f, point.first.y - verticalOffset }, Helpers::calculateColor(config->misc.nadeDamagePredict) & IM_COL32_A_MASK, point.second.c_str());
			drawList->AddText({ point.first.x - horizontalOffset, point.first.y - verticalOffset - 1.f }, Helpers::calculateColor(config->misc.nadeDamagePredict), point.second.c_str());
		}
	}
}
