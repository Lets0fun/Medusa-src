...
void Triggerbot::run(UserCmd* cmd) noexcept
{
    ...
    const auto& cfg = config->legitbob[weaponIndex];
    ...
    if (now - lastTime < cfg.reactionTime / 1000.0f)
        return;
    ...
    if (!config->lgb.ignoreSmoke && memory->lineGoesThroughSmoke(startPos, endPos, 1))
        return;
    ...
    for (int i = 1; i <= interfaces->engine->getMaxClients(); i++)
    {
        ...
        for (int j = 0; j < Hitboxes::Max; j++)
        {
            ...
            if (AimbotFunction::hitboxIntersection(player.matrix.data(), j, set, startPos, endPos))
            {
                ...
                if (damage >= (config->lgb.killshot ? trace.entity->health() : cfg.minDamage) &&
                    AimbotFunction::hitChance(localPlayer.get(), entity, set, player.matrix.data(), activeWeapon, AimbotFunction::calculateRelativeAngle(startPos, trace.endpos, cmd->viewangles + aimPunch), cmd, cfg.hitchanceT))
                {
        ...
        for (int j = 0; j < Hitboxes::Max; j++)
        {
            ...
            if (AimbotFunction::hitboxIntersection(record.matrix, j, set, startPos, endPos))
            {
                ...
                if (damage >= (config->lgb.killshot ? trace.entity->health() : cfg.minDamage) &&
                    AimbotFunction::hitChance(localPlayer.get(), entity, set, record.matrix, activeWeapon, AimbotFunction::calculateRelativeAngle(startPos, trace.endpos, cmd->viewangles + aimPunch), cmd, cfg.hitchanceT))
                {
                    cmd->buttons |= UserCmd::IN_ATTACK;
