...
#include "../Shaders/Build/chromatic_aberration.h"
...
#include "Resources/Shaders/passthrough.glsl"
    ...
    void _draws(ImDrawList* drawList, const ImVec2& p_min, const ImVec2& p_max, float alpha) noexcept
    {
        ...
        for (int i = 0; i < 8; ++i) {
            drawList->AddCallback(&firstPass, nullptr);
            drawList->AddImage(reinterpret_cast<ImTextureID>(blurTexture1), p_min, p_max, { 1.0f, 1.0f });
            drawList->AddCallback(&secondPass, nullptr);
            drawList->AddImage(reinterpret_cast<ImTextureID>(blurTexture2), p_min, p_max, { 1.0f, 1.0f });
        }
        drawList->AddCallback(&end, nullptr);
        drawList->AddCallback(ImDrawCallback_ResetRenderState, nullptr);
